...
- **Episode categories: series + tags**
  - `series` table: each episode belongs to one series (e.g., "The Squiz Today", "BBC 6 Minute English")
  - `tags` table + `episode_tags` join table: each episode can have multiple topic tags (e.g., "Politics", "Grammar")
  - iOS: group episodes by series, filter by tag
  - Admin: manage series and tags, assign tags to episodes
  - Deferred per YAGNI — v0.1–v0.4 only has one RSS feed, no categorization needed
- A-B loop (repeat segment) for listening practice
- Vocabulary bookmarking / word bank
...
- Background download
- Auto-split long audio (>45min) for transcription to avoid Edge Function timeout
